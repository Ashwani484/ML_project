...
class DataIngestion:
    ...
    '''
    # download csv file from gcp (google cloud platform)
    ...
    #loading the input dataset from local directory
    def load_input_data(self):
        try:
            logger.info("Loading the input data")
            return pd.read_csv(RAW_FILE_PATH)
        except Exception as e:
            logger.error("Error while loading the input data")
            raise CustomException("Failed to load input data ", e)
    ...
    def split_data(self):
        try:
            ...
            # saving the data to raw directory
